...
use crate::auth::{Claim, LoggedUser};
...
impl Token {
    pub fn create_token(data: &LoggedUser) -> Result<Self, ServiceError> {
        ...
        .map(Into::into)
        .map_err(|_err| ServiceError::InternalServerError(String::from("Could not verify Token")))
    }
    ...
    pub fn decode_token(token: &Self) -> Result<Claim, ServiceError> {
        ...
        .map_err(|_err| ServiceError::Unauthorized(String::from("Unauthorized Operation")))?
    }
}
